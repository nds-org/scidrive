...
public class ConflictException extends WebApplicationException {
	...
	public ConflictException(Throwable cause) {
		super(cause, Response.status(Status.CONFLICT).entity(cause.getMessage()).type("text/plain").build());
	}

	public ConflictException(Throwable cause, String message) {
		super(cause, Response.status(Status.CONFLICT).entity(message).type("text/plain").build());
	}
}
